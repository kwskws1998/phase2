...
def _compute_learned_heteroscedastic_sequential(logits, log_variance, labels, T):
    ...
    for t in range(T):
        ...
        # Memory-optimized: separate noise + del to keep peak at 3 tensors instead of 4
        # x_hat = logits + sigma * epsilon_t  would peak at 4 tensors (42.8 GB in bf16)
        # This approach peaks at 3 tensors (32 GB in bf16)
        noise = sigma * epsilon_t   # Peak: logits + epsilon_t + noise = 3 tensors
        del epsilon_t               # Now: logits + noise = 2 tensors
        x_hat = logits + noise      # Peak: logits + noise + x_hat = 3 tensors
        del noise                   # Now: logits + x_hat = 2 tensors
